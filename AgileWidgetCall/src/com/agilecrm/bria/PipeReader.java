...
import com.agilecrm.bria.enums.MessageDirection;

class PipeReader {

	private RandomAccessFile pipeFile;
	private Executor readingExecutor;
	private BlockingQueue<Message> messageQueue;

	public PipeReader(RandomAccessFile pipeFile, BlockingQueue<Message> messageQueue) {
		this.pipeFile = pipeFile;
		this.readingExecutor = Executors.newSingleThreadExecutor();
		this.messageQueue = messageQueue;
	}

	public void startReadingPipe() {
			...
			public void run() {
				...
				while (true) {
					try {
						...
						// Unfortunately polling seems to be the only way to
						// monitor the pipeFile for data. JDK 7 introduces
						// the WatchService API that may make this more elegant.
						if (pipeFile.length() <= 0) {
							try {
								Thread.sleep(300);
								continue;
						...
						int size = (int) (pipeFile.length() - pipeFile.getFilePointer());
						byte[] buffer = new byte[size];

						pipeFile.read(buffer);
		...
		this.readingExecutor.execute(readingRunnable);
	}
