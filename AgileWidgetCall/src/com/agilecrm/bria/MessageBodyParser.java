...
import com.agilecrm.bria.data.CallOptions;
import com.agilecrm.bria.data.Participant;
import com.agilecrm.bria.data.PhoneStatus;
import com.agilecrm.bria.data.VoicemailAccount;
import com.agilecrm.bria.enums.AccountStatus;
import com.agilecrm.bria.enums.CallHistoryEntryType;
import com.agilecrm.bria.enums.CallHoldStatusType;
import com.agilecrm.bria.enums.CallType;
import com.agilecrm.bria.enums.ParticipantStateType;
...
public class MessageBodyParser {
	...
	public static int parseMissedCallStatusResponse(Document missedCallStatusBody) {
		...
		Message callHistoryMessage = new MessageBuilder().callHistory(1, CallType.MISSED).build();
	...
	public static List<Call> parseCallStatusResponse(Document callMessageBody) {
		...
		callMessageBody.getDocumentElement().normalize();

		NodeList child = callMessageBody.getChildNodes();
		if(child != null){
				...
				for (int j = 0; j < participantList.getLength(); j++) {
					...
					if (participantTag.hasChildNodes()) {
						...
						if(state.equals("ringing")){
							//connect to website and show popup
							System.out.println("connect to website and show popup");
						}
